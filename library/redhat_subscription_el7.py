...
DOCUMENTATION = r"""
module: redhat_subscription_el7
...
class Rhsm(object):
    ...
    def _has_dbus_interface(self):
        ...
        return (distro_version[0] == 7 and distro_version[1] >= 4) or distro_version[
            0
        ] >= 8
    ...
    def _can_connect_to_dbus(self):
        ...
        try:
            ...
            self.module.debug(
                "Failed to connect to system D-Bus bus, will use CLI: %s" % e
            )
            return False
    ...
    def register(
        ...
        if (
            not token
            and not environment
            and self._has_dbus_interface()
            and self._can_connect_to_dbus()
        ):
    ...
    def _register_using_cli(
        ...
        rc, stderr, stdout = self.module.run_command(
            args, check_rc=True, expand_user_and_vars=False
        )
    ...
    def _register_using_dbus(
        ...
        if activationkey:
            ...
            dbus_has_enable_content_option = (
                supports_enable_content_for_activation_keys()
            )
        else:
            ...
            def supports_enable_content_for_credentials():
                ...
                if distro_id == "rhel" and (
                    (distro_version[0] == 8 and distro_version[1] >= 6)
                    or distro_version[0] >= 9
                ):
                    return True
                ...
                if distro_id == "centos" and (
                    (
                        distro_version[0] == 8
                        and (distro_version[1] >= 6 or distro_version_parts[1] == "")
                    )
                    or distro_version[0] >= 9
                ):
                    return True
        ...
        if consumer_type:
            ...
            def supports_option_consumer_type():
                ...
                if distro_id == "rhel" and (
                    (distro_version[0] == 9 and distro_version[1] >= 2)
                    or distro_version[0] >= 10
                ):
                    return True
        ...
        connection_opts = {}
        # Wrap it as proper D-Bus dict
        connection_opts = dbus.Dictionary(
            connection_opts, signature="sv", variant_level=1
        )

        bus = dbus.SystemBus()
        register_server = bus.get_object(
            "com.redhat.RHSM1", "/com/redhat/RHSM1/RegisterServer"
        )
        ...
        self.module.run_command(
            [SUBMAN_CMD, "refresh"], check_rc=True, expand_user_and_vars=False
        )
        ...
        for pool_id, quantity in sorted(pool_ids.items()):
            if pool_id in available_pool_ids:
                ...
                self.module.fail_json(
                    msg="Pool ID: %s not in list of available pools" % pool_id
                )
        return pool_ids
    ...
    def update_subscriptions_by_pool_ids(self, pool_ids):
        ...
        for pool_id, quantity in sorted(pool_ids.items()):
            ...
            if (
                quantity is None
                and quantity_used == 0
                or quantity not in (None, 0, quantity_used)
            ):
        ...
        return {
            "changed": changed,
            "subscribed_pool_ids": list(missing_pools.keys()),
...
class RhsmPools(object):
    ...
    def _load_product_list(self, consumed=False):
        ...
        rc, stdout, stderr = self.module.run_command(
            args, check_rc=True, environ_update=lang_env
        )
...
class SysPurpose(object):
    ...
    def update_syspurpose(self, new_syspurpose):
        ...
        for key, value in new_syspurpose.items():
            if key in self.ALLOWED_ATTRIBUTES:
                ...
                raise KeyError(
                    "Attribute: %s not in list of allowed attributes: %s"
    ...
    def _write_syspurpose(self, new_syspurpose):
        ...
        with open(self.path, "w") as fp:
            fp.write(
                json.dumps(new_syspurpose, indent=2, ensure_ascii=False, sort_keys=True)
            )
...
def main():
    ...
    module = AnsibleModule(
        ...
        required_if=[
            ["force_register", True, ["username", "activationkey", "token"], True]
        ],
    )
    ...
    if syspurpose is not None:
        try:
            ...
            module.fail_json(
                msg="Failed to update syspurpose attributes: %s" % to_native(err)
            )
        ...
        if was_registered and not force_register:
            if syspurpose and "sync" in syspurpose and syspurpose["sync"] is True:
                try:
                    ...
                    module.fail_json(
                        msg="Failed to synchronize syspurpose attributes: %s"
                        % to_native(e)
                    )
            if pool_ids:
                try:
            ...
            try:
                ...
                module.fail_json(
                    msg="Failed to register with '%s': %s"
                    % (server_hostname, to_native(e))
                )
            else:
                module.exit_json(
    ...
    if state == "absent":
        if not rhsm.is_registered:
            ...
            try:
                ...
                module.exit_json(
                    changed=True,
                    msg="System successfully unregistered from %s." % server_hostname,
                )
