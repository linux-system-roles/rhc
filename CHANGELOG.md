...
- feat: Argument spec implementation for rhc role (#310)

### Bug Fixes

- fix: Select RHSM modules by managed node python version (#299)
- fix: Move subscription-manager module calls to shared include task files (#309)

### Other Changes

- ci: use commit hash for github action, add persist-credentials false [citest_skip] (#296)
- ci: refactor build_docs so that pandoc runs in isolated read-only job [citest_skip] (#297)
- ci: use exact match for systemroller instead of contains [citest_skip] (#298)
- ci(build_docs): fix pandoc container syntax [citest_skip] (#301)
- ci: [citest_skip] Bump actions/download-artifact from 7.0.0 to 8.0.1 (#302)
- ci: [citest_skip] Bump codespell-project/actions-codespell from 2.1 to 2.2 (#303)
- ci: [citest_skip] Bump myrotvorets/set-commit-status-action from 2774e1f040c82ed70a76b4b5cd53bb11ffaedd0a to c0f880c99d91381c6fdb97726f03feb8004409b4 (#304)
- refactor: Relax collection constraints, gate vendored modules by python version, update ci versions [citest_skip] (#305)
- ci: do not run ci tests by default, require citest comment or label [citest_skip] (#306)
- ci: replace weekly_ci with periodic_ci, stagger schedules [citest_skip] (#307)
- ci: update status when action triggered by issue comment (#308)
- chore: Fix pep8 E501 line too long in rhsm_repository_python_36.py (#311)
...
- feat: Write roles fingerprints to /var/log/sysroles.jsonl [citest_skip] (#294)

### Other Changes

- ci: Bump actions/checkout from 6 to 7 (#287)
- ci: Use our own pr_title_lint.py instead of NPM commitlint [citest_skip] (#288)
- ci: bump tox-lsr version to 3.20.0 to fix tox 4.58 api breakage [citest_skip] (#289)
- ci: Add support for Fedora 44 and drop Fedora 42 - use ansible-core 2.21 [citest_skip] (#291)
- ci: Bump actions/setup-python from 6 to 7 (#292)
- ci: ensure dependabot updates do not invoke ci tests [citest_skip] (#293)
...
- refactor: use ansible.posix 2.1.X for EL7 compatibility [citest_skip] (#282)
- ci: Add config file for CodeRabbit with custom rules (#283)
- ci: Skip reviews for PRs with [citest_skip] in the title (#284)
- test: remove unused files [citest_skip] (#285)
...
- ci: Bump actions/github-script from 8 to 9 (#275)
...
- ci: use ANSIBLE_INJECT_FACT_VARS=false by default for testing (#255)
- ci: Bump ansible/ansible-lint from 25 to 26 (#256)
- ci: skip most CI checks if title contains citest skip [citest_skip] (#257)
- ci: ansible-lint - remove .collection directory from converted collection [citest_skip] (#258)
- ci: tox-lsr version 3.15.0 [citest_skip] (#259)
- ci: Add Fedora 43, remove Fedora 41 from Testing Farm CI (#260)
- ci: Bump actions/upload-artifact from 6 to 7 (#262)
- ci: tox-lsr 3.17.0 - container test improvements, use ansible 2.20 for fedora 43 [citest_skip] (#265)
- test: support an external proxy (#266)
- ci: tox-lsr 3.17.1 - previous update broke container tests, this fixes them [citest_skip] (#267)
- test: ensure role gathers the facts it uses by having test clear_facts before include_role (#268)
- ci: fix yum repos to use devel site instead of old site name [citest_skip] (#269)
- ci: update header for run_role_with_clear_facts [citest_skip] (#270)
- ci: Comply with Ansible partner certification checking [citest_skip] (#271)
- ci: ansible-lint requires dependencies to be installed [citest_skip] (#272)
